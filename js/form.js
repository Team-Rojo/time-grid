...
const daysDropdownButton = document.querySelector(
  "#days-dropdown-button"
);
const daysDropdownMenu = document.querySelector(
  "#days-dropdown-menu"
);
...
function toggleDaysDropdown() {
  ...
  daysDropdownButton.setAttribute(
    "aria-expanded",
    String(isOpen)
  );
}
...
export function initializeForm(activities) {
  ...
  activityForm.addEventListener("submit", (event) => {
    ...
    daysDropdownButton.addEventListener(
    "click",
    toggleDaysDropdown
  );
...
document.addEventListener("click", (event) => {
  const clickedInsideDropdown =
    event.target.closest(".days-dropdown");
